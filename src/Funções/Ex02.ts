...
function emitir_mensagem_alerta(status_alerta: boolean): void {
  if (status_alerta) {
    document.writeln("ALERTA: Temperatura fora da faixa ideal!<br>");
  } else {
    document.writeln("Temperatura dentro da faixa normal.<br>");
  }
}

let temperatura = ler_temperatura();
let statusalert = verificar_alerta_temperatura(temperatura);
emitir_mensagem_alerta(statusalert);